...
const TABLE_MAP = {
  ...
  [SHEET_NAMES.subjectObservations]: 'subject_observations',
  [SHEET_NAMES.directorObservations]: 'director_observations',
...
const SELECT_MAP = {
  ...
  [SHEET_NAMES.teachers]: 'id,sede_id,first_name,last_name,email,password_hash,is_admin,active',
  ...
  [SHEET_NAMES.preReports]:
    'id,period_id,grade_id,subject_id,teacher_id,student_id,convivencia,academica,status,created_at,updated_at,deleted_at',
  [SHEET_NAMES.subjectObservations]:
    'id,period_id,grade_id,subject_id,teacher_id,student_id,observations,status,created_at,updated_at,deleted_at',
  [SHEET_NAMES.directorObservations]:
    'id,period_id,grade_id,director_teacher_id,student_id,observations,status,created_at,updated_at,deleted_at',
...
function mapDbToApp(sheetName, row) {
  switch (sheetName) {
    ...
    case SHEET_NAMES.subjectObservations:
      return {
        id: row.id,
        periodId: row.period_id || '',
        gradeId: row.grade_id || '',
    ...
    case SHEET_NAMES.directorObservations:
      return {
        id: row.id,
        periodId: row.period_id || '',
        gradeId: row.grade_id || '',
  ...
  switch (sheetName) {
    case SHEET_NAMES.teachers:
      return {
        ...
        email: normalizeCell(record.email).toLowerCase() || null,
    ...
    case SHEET_NAMES.subjectObservations:
      return {
        ...
        teacher_id: normalizeCell(record.teacherId),
        student_id: normalizeCell(record.studentId),
        observations: normalizeRichText(record.observations),
        status: normalizeCell(record.status || 'active'),
        created_at: normalizeCell(record.createdAt || nowIso()),
        updated_at: normalizeCell(record.updatedAt || nowIso()),
        deleted_at: normalizeCell(record.deletedAt) || null
...
export function normalizeTeacherRecord(payload, existing = {}) {
  return {
    ...
    email: normalizeCell(payload.email ?? existing.email).toLowerCase(),
...
export function normalizeSubjectObservationRecord(payload, existing = {}) {
  const status = payload.status || existing.status || 'active';
  return {
    id: normalizeCell(payload.id || existing.id),
    periodId: normalizeCell(payload.periodId || existing.periodId),
...
export function normalizeDirectorObservationRecord(payload, existing = {}) {
  const status = payload.status || existing.status || 'active';
  return {
    id: normalizeCell(payload.id || existing.id),
    periodId: normalizeCell(payload.periodId || existing.periodId),
...
export async function deletePreReportsByPeriod(periodId) {
  await Promise.all(
    [SHEET_NAMES.preReports, SHEET_NAMES.subjectObservations, SHEET_NAMES.directorObservations].map((sheetName) =>
      supabaseRequest(getTableName(sheetName), {
        method: 'DELETE',
        query: {
  ...
  [SHEET_NAMES.preReports, SHEET_NAMES.subjectObservations, SHEET_NAMES.directorObservations].forEach((sheetName) =>
    clearCache(`sheet:${sheetName}`)
  );
...
export async function getDomainData() {
  const [
    ...
    getSheetRecords(SHEET_NAMES.subjectObservations),
    getSheetRecords(SHEET_NAMES.directorObservations)
  ]);

  const activeSubjectObservations = subjectObservations.filter((item) => item.status !== 'deleted');
  const activeDirectorObservations = directorObservations.filter((item) => item.status !== 'deleted');
  const subjectObservationByKey = new Map(
    activeSubjectObservations.map((item) => [
      `${item.periodId}::${item.gradeId}::${item.subjectId}::${item.teacherId}::${item.studentId}`,
      item
    ])
  );
  const directorObservationByKey = new Map(
    activeDirectorObservations.map((item) => [`${item.periodId}::${item.gradeId}::${item.studentId}`, item])
  );
  const combinedPreReports = preReports
    .filter((item) => item.status !== 'deleted')
    .map(inflatePreReport)
    .map((item) => ({
      ...item,
      observations:
        subjectObservationByKey.get(
          `${item.periodId}::${item.gradeId}::${item.subjectId}::${item.teacherId}::${item.studentId}`
        )?.observations || '',
      directorObservations:
        directorObservationByKey.get(`${item.periodId}::${item.gradeId}::${item.studentId}`)?.observations || ''
    }));
  const preReportKeys = new Set(
    combinedPreReports.map(
      (item) => `${item.periodId}::${item.gradeId}::${item.subjectId}::${item.teacherId}::${item.studentId}`
    )
  );
  for (const observation of activeSubjectObservations) {
    const key = `${observation.periodId}::${observation.gradeId}::${observation.subjectId}::${observation.teacherId}::${observation.studentId}`;
    if (preReportKeys.has(key)) continue;
    combinedPreReports.push({
      id: `subject-observation:${observation.id}`,
      periodId: observation.periodId,
      gradeId: observation.gradeId,
      ...
      directorObservations:
        directorObservationByKey.get(`${observation.periodId}::${observation.gradeId}::${observation.studentId}`)
          ?.observations || '',
      status: 'active',
  ...
  return {
    ...
    students,
    periods,
    preReports: combinedPreReports,
    subjectObservations: activeSubjectObservations,
    directorObservations: activeDirectorObservations
  };
}
