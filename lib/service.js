...
function mapTeacherView(data, session) {
  ...
  return {
    ...
    students: sortStudentsByLastName(session.isAdmin ? data.students : data.students.filter((item) => gradeIds.has(item.gradeId))),
  ...
  return {
    ...
    adminView: session.isAdmin
      ? {
          ...
          students: sortStudentsByLastName(adminRecords[6]),
