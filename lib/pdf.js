...
function normalizeHtmlColor(value) {
...
function extractTagColor(tag) {
  const styleMatch = tag.match(/\bstyle\s*=\s*["'][^"']*\bcolor\s*:\s*([^;"']+)/i);
  const attributeMatch = tag.match(/\bcolor\s*=\s*["']?([^"'\s>]+)/i);
  return normalizeHtmlColor(styleMatch?.[1] || attributeMatch?.[1] || '');
}

function extractStyledObservationParagraphs(value) {
  const source = String(value || '').replace(/\r\n/g, '\n');
  if (!source.trim()) return [];
  if (!/<\/?[a-z][\s\S]*>/i.test(source)) {
    return source
      .split(/\n+/)
      .map((paragraph) => [{ text: sanitizeText(paragraph.trim()), color: '', bold: false, italic: false, underline: false }])
  ...
  const paragraphs = [];
  const listStack = [];
  const styleStack = [{ color: '', bold: false, italic: false, underline: false }];
  let currentRuns = [];
  ...
  function appendRun(text, style = getCurrentStyle()) {
    const normalized = decodeHtmlEntities(text).replace(/\s+/g, ' ').trim();
    if (!normalized) return;
    const previous = currentRuns[currentRuns.length - 1];
    const prefix = previous && !previous.text.endsWith(' ') ? ' ' : '';
    if (hasSameStyle(previous, style)) {
      previous.text += `${prefix}${normalized}`;
    } else {
      currentRuns.push({ text: `${prefix}${normalized}`, ...style });
    }
  }

  function flushParagraph() {
    if (!currentRuns.length) return;
    currentRuns[0].text = currentRuns[0].text.trimStart();
    currentRuns[currentRuns.length - 1].text = currentRuns[currentRuns.length - 1].text.trimEnd();
    const cleaned = currentRuns.filter((run) => run.text);
  ...
  for (const token of tokens) {
    ...
    const tag = token.toLowerCase().replace(/\s+/g, ' ');
    const openingStyleTag = tag.match(/^<(span|font|b|strong|i|em|u)\b/);
    if (openingStyleTag) {
      const nextStyle = { ...getCurrentStyle() };
      if (openingStyleTag[1] === 'span' || openingStyleTag[1] === 'font') {
        nextStyle.color = extractTagColor(token) || nextStyle.color;
      } else if (openingStyleTag[1] === 'b' || openingStyleTag[1] === 'strong') {
        nextStyle.bold = true;
      } else if (openingStyleTag[1] === 'i' || openingStyleTag[1] === 'em') {
        nextStyle.italic = true;
      } else if (openingStyleTag[1] === 'u') {
        nextStyle.underline = true;
      }
      styleStack.push(nextStyle);
      continue;
    }
    if (/^<\/(span|font|b|strong|i|em|u)>/.test(tag)) {
      if (styleStack.length > 1) styleStack.pop();
      continue;
    }
    if (/^<br\b/.test(tag) || /^<(p|div)\b/.test(tag) || /^<\/(p|div)>/.test(tag)) {
      flushParagraph();
      continue;
    }
    if (/^<ul\b/.test(tag)) {
    ...
    if (/^<li\b/.test(tag)) {
      ...
      if (currentList?.type === 'ol') {
        currentList.index += 1;
        currentRuns = [{ text: `${currentList.index}.`, color: '', bold: false, italic: false, underline: false }];
      } else {
        currentRuns = [{ text: '•', color: '', bold: false, italic: false, underline: false }];
      }
      continue;
    }
    if (/^<\/li>/.test(tag)) flushParagraph();
  }
...
function getObservationRunFont(run, fonts) {
...
function hasSameObservationStyle(left, right) {
  return (
    left?.color === right?.color &&
    left?.bold === right?.bold &&
    left?.italic === right?.italic &&
...
function wrapObservationParagraphs(value, maxWidth, fonts, size) {
  const paragraphs = extractStyledObservationParagraphs(value);
  ...
  paragraphs.forEach((paragraph, index) => {
    ...
    for (const run of paragraph) {
      const words = sanitizeText(run.text).split(/\s+/).filter(Boolean);
      const runFont = getObservationRunFont(run, fonts);
      for (const word of words) {
        const prefix = currentLine.length ? ' ' : '';
        const text = `${prefix}${word}`;
        const width = runFont.widthOfTextAtSize(text, size);
        if (currentLine.length && currentWidth + width > maxWidth) {
          lines.push(currentLine);
          currentLine = [];
          currentWidth = 0;
        }
        const nextText = currentLine.length ? ` ${word}` : word;
        const previous = currentLine[currentLine.length - 1];
        if (hasSameObservationStyle(previous, run)) {
          previous.text += nextText;
        } else {
          currentLine.push({ text: nextText, color: run.color, bold: run.bold, italic: run.italic, underline: run.underline });
        }
        currentWidth += runFont.widthOfTextAtSize(nextText, size);
      }
    }
...
function pdfColorFromHex(value, fallback = rgb(0.12, 0.12, 0.12)) {
  const color = normalizeHtmlColor(value);
...
function drawObservationsPage(pdf, font, bold, italic, boldItalic, item, headerAssets) {
  ...
  for (const entry of item.observationEntries) {
    ...
    const observationFonts = { regular: font, bold, italic, boldItalic };
    const observationLines = wrapObservationParagraphs(entry.observations, maxWidth, observationFonts, bodySize);
    ...
    for (const line of observationLines) {
      ...
      for (const run of line) {
        ...
        const runFont = getObservationRunFont(run, observationFonts);
        const runColor = pdfColorFromHex(run.color);
        const textWidth = runFont.widthOfTextAtSize(text, bodySize);
        page.drawText(text, {
          x: currentX,
          y: currentY,
          size: bodySize,
        ...
        if (run.underline && text.trim()) {
          page.drawLine({
            start: { x: currentX, y: currentY - 1.2 },
            end: { x: currentX + textWidth, y: currentY - 1.2 },
            thickness: 0.6,
            color: runColor
          });
        }
...
export async function buildPreReportsPdf(items) {
  ...
  for (const item of items) {
    ...
    if (item.observationEntries?.length) {
      drawObservationsPage(pdf, font, bold, italic, boldItalic, item, headerAssets);
    }
  }
