...
import {
  ...
  getLatestPasswordResetForTeacher,
  resetTeacherPasswordWithToken
...
import {
  ...
  normalizeSubjectObservationRecord,
  normalizeDirectorObservationRecord,
  normalizeSedeRecord,
...
function validateTeacherEmail(value) {
...
function ensureUniqueTeacherEmail(teachers, teacherId, email) {
  if (!email) return;
  if (teachers.some((teacher) => teacher.id !== teacherId && normalizeEmail(teacher.email) === email)) {
    const error = new Error('El correo electrónico ya está registrado para otro docente');
    error.statusCode = 409;
    throw error;
  }
}

function ensurePasswordRecoveryConfig() {
  const missing = [];
  if (!config.supabaseServiceRoleKey) missing.push('SUPABASE_SERVICE_ROLE_KEY');
  if (!config.appBaseUrl) missing.push('APP_BASE_URL');
  if (!config.gmailSmtpUser) missing.push('GMAIL_SMTP_USER');
  if (!config.gmailAppPassword) missing.push('GMAIL_APP_PASSWORD');
  if (missing.length) {
    const error = new Error(`La recuperación de contraseña no está configurada (${missing.join(', ')})`);
...
function buildDirectorObservationPanel(data, session, periodId, gradeId) {
  ...
  const observationByStudentId = new Map(
    data.directorObservations
      .filter((item) => item.periodId === normalizedPeriodId && item.gradeId === grade.id && item.status !== 'deleted')
      .map((item) => [item.studentId, item.observations || ''])
  );

  const students = sortStudentsByLastName(
    data.students.filter((item) => item.gradeId === grade.id && item.active !== 'FALSE')
    ...
    accumulator.push({
      ...
      directorObservations: observationByStudentId.get(student.id) || '',
...
export async function requestPasswordResetService(login) {
  const userId = normalizeCell(login);
  if (!userId) {
    const error = new Error('Debes indicar tu usuario');
    error.statusCode = 400;
    throw error;
  }

  const teachers = await getSheetRecords(SHEET_NAMES.teachers);
  const teacher = teachers.find((item) => item.id === userId && item.active !== 'FALSE');
  if (!teacher) {
    const error = new Error('No existe un docente activo con ese usuario');
  ...
  if (!email) {
    const error = new Error('Este usuario no tiene un correo de recuperación registrado. Contacta al administrador.');
    error.statusCode = 409;
    throw error;
  }
  ensurePasswordRecoveryConfig();

  const latestReset = await getLatestPasswordResetForTeacher(teacher.id);
  const cooldownSeconds = Number.isFinite(config.passwordResetCooldownSeconds)
    ? Math.max(0, config.passwordResetCooldownSeconds)
    : 60;
  if (latestReset?.created_at && Date.now() - new Date(latestReset.created_at).getTime() < cooldownSeconds * 1000) {
    const error = new Error(`Espera ${cooldownSeconds} segundos antes de solicitar otro correo`);
    error.statusCode = 429;
    throw error;
  }

  const ttlMinutes = Number.isFinite(config.passwordResetTtlMinutes)
    ? Math.max(5, config.passwordResetTtlMinutes)
    : 30;
  ...
  await createPasswordResetRecord({ tokenHash, teacherId: teacher.id, expiresAt });
  try {
    await sendPasswordResetEmail({
      email,
      teacherName: getTeacherDisplayName(teacher) || teacher.id,
      resetUrl: resetUrl.toString(),
      expiresInMinutes: ttlMinutes
    });
  } catch (error) {
    await deletePasswordResetRecord(tokenHash).catch((cleanupError) => {
      console.error('No fue posible limpiar el token de recuperación:', cleanupError);
    });
    throw error;
  }

  logAuditEvent('password_reset_requested', { userId: teacher.id }, { email: maskEmail(email) });
  return {
    message: `Enviamos el enlace de recuperación a ${maskEmail(email)}.`,
    expiresInMinutes: ttlMinutes
  };
}

export async function resetPasswordService(token, password) {
  const normalizedToken = normalizeCell(token);
  const normalizedPassword = normalizeCell(password);
  if (!normalizedToken) {
    const error = new Error('El enlace de recuperación no es válido');
    error.statusCode = 400;
    throw error;
  }
  if (normalizedPassword.length < MIN_PASSWORD_LENGTH) {
    const error = new Error(`La nueva contraseña debe tener al menos ${MIN_PASSWORD_LENGTH} caracteres`);
    error.statusCode = 400;
    throw error;
  }
  if (normalizedPassword.length > 128) {
    const error = new Error('La nueva contraseña es demasiado larga');
    error.statusCode = 400;
    throw error;
  }
  if (!config.supabaseServiceRoleKey) {
    const error = new Error('La recuperación de contraseña no está configurada (SUPABASE_SERVICE_ROLE_KEY)');
    error.statusCode = 503;
    throw error;
  }

  const teacherId = await resetTeacherPasswordWithToken(
    hashSecureToken(normalizedToken),
    hashPassword(normalizedPassword, config.passwordSalt)
  );
  if (!teacherId) {
  ...
  logAuditEvent('password_reset_completed', { userId: teacherId });
...
export async function saveEntityService(entityName, payload) {
  ...
  switch (entityName) {
    ...
    case SHEET_NAMES.teachers:
      ...
      normalized.email = validateTeacherEmail(normalized.email);
      ensureUniqueTeacherEmail(records, normalized.id, normalized.email);
      break;
...
export async function importConfigWorkbookService(entityType, rows) {
  ...
  if (entityType === SHEET_NAMES.teachers) {
    const importedEmails = new Set();
    for (const row of rows) {
      const email = validateTeacherEmail(row.email);
      const rowId = normalizeCell(row.id);
      ensureUniqueTeacherEmail(records, rowId, email);
      if (email && importedEmails.has(email)) {
        const error = new Error('La importación contiene correos electrónicos duplicados');
        error.statusCode = 409;
        throw error;
      }
...
function getTeacherAssignmentOrThrow(data, session, gradeId, subjectId) {
  const effectiveTeacherId = getEffectiveTeacherId(session);
  const assignment = data.gradeSubjects.find(
    (item) =>
      item.teacherId === effectiveTeacherId &&
  ...
  if (!assignment) {
    const error = new Error('No tiene acceso a esta asignación');
    error.statusCode = 403;
    throw error;
  }
...
function validateTeacherWorkspaceFilters(periodId, gradeId, subjectId) {
  if (!normalizeCell(periodId) || !normalizeCell(gradeId) || !normalizeCell(subjectId)) {
    const error = new Error('Debe seleccionar período, grado y asignatura');
    error.statusCode = 400;
    throw error;
  }
}

export async function getTeacherPreReportWorkspaceService(session, filters = {}) {
  const periodId = normalizeCell(filters.periodId);
  const gradeId = normalizeCell(filters.gradeId);
  const subjectId = normalizeCell(filters.subjectId);
  validateTeacherWorkspaceFilters(periodId, gradeId, subjectId);

  const data = await getDomainData();
  const effectiveTeacherId = getTeacherAssignmentOrThrow(data, session, gradeId, subjectId);
  ...
  return {
    students: sortStudentsByLastName(data.students.filter((item) => item.gradeId === gradeId && item.active !== 'FALSE')).map(
      (student) => {
        ...
        return {
          ...
          firstName: student.firstName,
          lastName: student.lastName,
          convivencia: report?.status === 'deleted' ? [] : report?.convivencia || [],
          academica: report?.status === 'deleted' ? [] : report?.academica || [],
          observations: report?.status === 'deleted' ? '' : report?.observations || ''
        };
      }
    )
  };
...
export async function saveTeacherPreReportMarksService(session, payload = {}) {
  const periodId = normalizeCell(payload.periodId);
  const gradeId = normalizeCell(payload.gradeId);
  const subjectId = normalizeCell(payload.subjectId);
  const rows = Array.isArray(payload.rows) ? payload.rows : [];
  ...
  const data = await getDomainData();
  const effectiveTeacherId = getTeacherAssignmentOrThrow(data, session, gradeId, subjectId);
  const students = data.students.filter((item) => item.gradeId === gradeId && item.active !== 'FALSE');
  const allowedStudentIds = new Set(students.map((item) => item.id));
  const incomingByStudentId = new Map();
  ...
  for (const row of rows) {
    ...
    incomingByStudentId.set(studentId, {
      convivencia: Array.isArray(row.convivencia) ? row.convivencia : [],
      academica: Array.isArray(row.academica) ? row.academica : []
    });
  }

  const rawPreReports = await getSheetRecords(SHEET_NAMES.preReports);
  ...
  for (const student of students) {
    ...
    const marks = incomingByStudentId.get(student.id) || { convivencia: [], academica: [] };
    const hasMarks = Boolean(marks.convivencia.length || marks.academica.length);
    if (!existing && !hasMarks) continue;
    ...
    const next = {
      ...
      teacherId: effectiveTeacherId,
      studentId: student.id,
      convivencia: marks.convivencia,
      academica: marks.academica,
      status: 'active'
    };
    if (!hasMarks) {
      next.status = 'deleted';
    ...
    recordsToSave.push(normalizePreReportRecord(next, existing || {}));
  }

  if (recordsToSave.length) {
    await appendRecords(SHEET_NAMES.preReports, recordsToSave);
  }
  return { saved: recordsToSave.length, marked, cleared };
}

export async function saveTeacherSubjectObservationsService(session, payload = {}) {
  const periodId = normalizeCell(payload.periodId);
  const gradeId = normalizeCell(payload.gradeId);
  const subjectId = normalizeCell(payload.subjectId);
  const rows = Array.isArray(payload.rows) ? payload.rows : [];
  ...
  for (const row of rows) {
    ...
    incomingByStudentId.set(studentId, normalizeRichText(row.observations));
  }

  const subjectObservationRecords = await getSheetRecords(SHEET_NAMES.subjectObservations);
  const existingByStudentId = new Map(
    subjectObservationRecords
      .filter(
        (item) =>
          item.teacherId === effectiveTeacherId &&
          item.periodId === periodId &&
  ...
  const recordsToSave = [];
  for (const [studentId, observations] of incomingByStudentId) {
    const existing = existingByStudentId.get(studentId);
    if (!existing && !observations) continue;
    const next = normalizeSubjectObservationRecord({
      periodId,
      gradeId,
      subjectId,
      teacherId: effectiveTeacherId,
      studentId,
      observations,
      status: observations ? 'active' : 'deleted'
    }, existing || {});
    recordsToSave.push(next);
  }
  if (recordsToSave.length) {
    await appendRecords(SHEET_NAMES.subjectObservations, recordsToSave);
  }
  return { saved: recordsToSave.length };
}
...
export async function saveDirectorObservationsService(session, payload = {}) {
  ...
  const perStudentMap =
    mode === 'per_student'
      ...
      : null;
  const directorObservationRecords = await getSheetRecords(SHEET_NAMES.directorObservations);
  const existingByStudentId = new Map(
    directorObservationRecords
      .filter((item) => item.periodId === panel.periodId && item.gradeId === panel.gradeId)
      .map((item) => [item.studentId, item])
  );
  const directorTeacherId = normalizeCell(
    data.grades.find((item) => item.id === panel.gradeId)?.directorTeacherId || getEffectiveTeacherId(session)
  );
  const nextRecords = panel.students.map((student) => {
    const existing = existingByStudentId.get(student.studentId);
    const observations =
      mode === 'per_student'
        ? perStudentMap?.get(student.studentId) ?? existing?.observations ?? ''
        : normalizeRichText(payload.observation);
    return normalizeDirectorObservationRecord(
      {
  ...
  if (nextRecords.length) {
    await appendRecords(SHEET_NAMES.directorObservations, nextRecords);
  }
