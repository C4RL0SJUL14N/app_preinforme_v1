...
function drawDocumentHeader(page, assets, font, bold) {
  ...
  drawCell(page, 'Institucion Educativa Juan de Dios Giron', col1X, topY, centerWidth, rowHeights[0], {
    font,
    boldFont: bold,
    bold: true,
  ...
  drawCell(page, `Proceso de Acompanamiento del Ano Escolar ${new Date().getFullYear()}`, col1X, topY - rowHeights[0], centerWidth, rowHeights[1], {
    font,
    boldFont: bold,
    bold: true,
...
function drawNoDataPage(pdf, font, bold, headerAssets) {
  ...
  page.drawText('Ajusta periodo, grado o estudiante y vuelve a intentar.', {
    x: 48,
    y: headerBottomY - 70,
    size: 11,
    font,
...
function drawObservationsPage(pdf, font, bold, item, headerAssets) {
  ...
  for (const entry of item.observationEntries) {
    const subjectLines = splitLines(`Asignatura: ${entry.subjectName}`, maxWidth, bold, 10);
    const observationLines = wrapObservationParagraphs(entry.observations, maxWidth, font, 10);
    const teacherLines = splitLines(`Docente: ${entry.teacherName}`, maxWidth, font, 10);
    const blockHeight = 12 * (subjectLines.length + observationLines.length + teacherLines.length) + 24;
...
export async function buildPreReportsPdf(items) {
  ...
  for (const item of items) {
    ...
    drawRow(
      page,
      [
        { text: `Periodo: ${item.periodName || ''}`, width: 160, bold: true, fontSize: 8 },
    ...
    drawRow(
      page,
      [
        { text: 'Areas curriculares', width: FIRST_COLUMN_WIDTH, bold: true, fontSize: 8, align: 'center', fillColor: HEADER_FILL },
