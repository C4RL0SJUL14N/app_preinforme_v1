...
export async function saveTeacherPreReportMarksService(session, payload = {}) {
  ...
  for (const student of students) {
    ...
    if (!hasMarks && !normalizeRichText(existing?.observations) && !normalizeRichText(existing?.directorObservations)) {
      next.status = 'deleted';
      cleared += 1;
    } else if (hasMarks) {
      marked += 1;
...
export async function saveTeacherSubjectObservationsService(session, payload = {}) {
  ...
  for (const student of students) {
    const existing = existingByStudentId.get(student.id);
    const observations = incomingByStudentId.get(student.id) || '';
    ...
    if (
      !observations &&
      !(existing?.convivencia || []).length &&
      !(existing?.academica || []).length &&
      !normalizeRichText(existing?.directorObservations)
    ) {
      next.status = 'deleted';
    }
    recordsToSave.push(normalizePreReportRecord(next, existing || {}));
...
export async function saveDirectorObservationsService(session, payload = {}) {
  ...
  const nextRecords = currentRecords.map((item) => {
    ...
    const inflatedItem = inflatePreReport(item);
    ...
    return normalizePreReportRecord({ ...inflatedItem, directorObservations: nextDirectorObservations }, item);
  });
