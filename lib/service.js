...
import { dedupeBy, hashPassword, normalizeCell, normalizeRichText } from './utils.js';
...
function buildReportScope(data, session, filters = {}) {
  ...
  const visibleStudents = data.students.filter((item) => {
    ...
    if (canUseAdminWideScope(session)) return true;
  ...
  return Array.from(grouped.values()).map((reports) => {
    ...
    const rows = [
      ...
      ...CONVIVENCIA_QUESTIONS.map((question) => ({
        type: 'question',
        label: question,
        marks: subjects.map((subject) => Boolean(reportBySubject.get(subject.reportLookupId || subject.subjectId)?.convivencia?.includes(question)))
      })),
      { type: 'section', label: 'Dificultades académicas' },
      ...ACADEMICA_QUESTIONS.map((question) => ({
        type: 'question',
        label: question,
        marks: subjects.map((subject) => Boolean(reportBySubject.get(subject.reportLookupId || subject.subjectId)?.academica?.includes(question)))
      }))
    ];
...
export async function createPreReportService(session, payload) {
  ...
  const normalized = normalizePreReportRecord({ ...payload, teacherId: effectiveTeacherId });
...
export async function copyTeacherPreReportsBetweenSubjectsService(session, payload) {
  ...
  const normalizedRows = rowsToCopy.map((item) =>
    normalizePreReportRecord({
      periodId,
      gradeId,
  ...
  await saveSheetRecords(SHEET_NAMES.preReports, [...currentRecords, ...normalizedRows]);
