...
export async function buildAdminSummaryPdf(summary) {
  ...
  const drawMetricGroup = (page, currentY, title, items) => {
    page.drawText(sanitizeText(title), {
      x: MARGIN,
      y: currentY,
      size: 10.5,
    ...
    if (!items.length) {
      page.drawText('Sin datos para mostrar.', {
        x: MARGIN + 4,
        y: currentY,
        size: 8.5,
        font,
    ...
    for (const item of items) {
      ...
      page.drawText(sanitizeText(`${label}${suffix}`), {
        x: MARGIN + 4,
        y: currentY,
        size: 8.8,
        font,
  ...
  for (const section of sections) {
    ...
    currentY = drawMetricGroup(page, currentY, section.title, section.items);
  }
