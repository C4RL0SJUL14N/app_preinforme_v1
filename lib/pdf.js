...
function extractStyledObservationParagraphs(value) {
  ...
  if (!/<\/?[a-z][\s\S]*>/i.test(source)) {
    return source
      .split(/\n+/)
      .map((paragraph) => [{ text: sanitizeText(paragraph.trim()), color: '' }])
  ...
  function appendRun(text, color = colorStack[colorStack.length - 1]) {
    const normalized = decodeHtmlEntities(text).replace(/\s+/g, ' ').trim();
    if (!normalized) return;
    ...
    if (previous?.color === color) {
      ...
      currentRuns.push({ text: `${prefix}${normalized}`, color });
    }
  }
  ...
  for (const token of tokens) {
    ...
    const tag = token.toLowerCase().replace(/\s+/g, ' ');
    if (/^<(span|font)\b/.test(tag)) {
      colorStack.push(extractTagColor(token) || colorStack[colorStack.length - 1]);
      continue;
    }
    if (/^<\/(span|font)>/.test(tag)) {
    ...
    if (/^<li\b/.test(tag)) {
      ...
      if (currentList?.type === 'ol') {
        currentList.index += 1;
        currentRuns = [{ text: `${currentList.index}.`, color: '' }];
      } else {
        currentRuns = [{ text: '-', color: '' }];
      }
      continue;
    }
    ...
    for (const run of paragraph) {
      ...
      for (const word of words) {
        ...
        if (previous?.color === run.color) {
          ...
          currentLine.push({ text: nextText, color: run.color });
        }
        currentWidth += font.widthOfTextAtSize(nextText, size);
      }
    }
    ...
    for (const line of observationLines) {
      ...
      for (const run of line) {
        ...
        page.drawText(text, {
          ...
          size: bodySize,
          font,
          color: pdfColorFromHex(run.color)
        });
        currentX += font.widthOfTextAtSize(text, bodySize);
      }
      currentY -= lineStep;
    }
