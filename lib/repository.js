...
import { ensureArray, normalizeBooleanString, normalizeCell, nowIso, parseJsonSafe } from './utils.js';
...
async function fetchTable(sheetName) {
  ...
  return (rows || []).map((row) => mapDbToApp(sheetName, row));
}
...
export function normalizePreReportRecord(payload, existing = {}) {
  ...
  return {
    ...
    observations: normalizeCell(payload.observations || existing.observations),
...
export async function getDomainData() {
  const [institutions, sedes, teachers, subjects, grades, gradeSubjects, students, periods, preReports] = await Promise.all([
